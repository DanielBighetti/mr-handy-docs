...
> **Adjusting for a new company?** You can put the Mr. Handy folder inside any existing repo, or keep it as a standalone tool folder that other projects include. The wizard rewrites paths relative to wherever it runs.
...
5. Detects which IDEs you have installed (Cursor, Claude Code, Codex, Windsurf, Zed, Cline, Continue) and writes the right `mcp.json` to each. Figma and Atlassian use OAuth — no PAT, no email, no API token is ever asked for.
...
## 3 · Sign in to Figma and Atlassian (OAuth — no PAT needed)
...
(You will still import the **uSpec Extract** plugin in Step 9 below — that's a separate, vendored plugin used only by the per-component path.)
...
The portable skill reuses the existing `.cursor/agents/*.md` specs as subagent prompts, so behavior stays consistent with the Cursor path.
...
This path needs the **uSpec Extract** plugin — Mr. Handy's vendored extractor. The setup wizard built it for you under `figma-plugin/dist/`. One-time import in Figma Desktop:
...
2. **Replace the Figma library key** in `mr-handy.config.json`:
   ...
   ```
3. **Re-run the wizard** so the new library key and any new IDE paths are wired into the per-IDE `mcp.json` files. Figma and Atlassian auth stay OAuth — no secrets to rotate on a handoff.
...
If the new company uses a custom design system that is not in the built-in list, pick `custom` in the wizard and add a reference doc to `Knowledge/design-systems/{your-ds}.md` following the `_template.md` pattern.
...
Pull from the repo. If `mr-handy.config.json.schema` changes, re-run the setup wizard (`Setup-MrHandy.bat` on Windows, `./setup-mrhandy.sh` on macOS / Linux, or `node setup-mrhandy.mjs` directly) — it preserves your answers and only patches new fields.
